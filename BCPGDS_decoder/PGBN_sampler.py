...
def Multrnd_Matrix_wcj(X_t, Phi_t, Theta_t):
    ...
    WSZS_t = np.zeros([V, K], dtype=np.float32)
    Xt_to_t1_t = np.zeros([K, J], dtype=np.float32)


    Phi_tmp = Phi_t[X_t_rows, :]
    Theta_tmp = Theta_t[:, X_t_cols]
    Phi_Theta = Phi_tmp * np.transpose(Theta_tmp) * np.reshape(X_t_values, [N, 1])

    WSZS_t[X_t_rows, :] = WSZS_t[X_t_rows, :] + Phi_Theta
    Xt_to_t1_t[:, X_t_cols] += Xt_to_t1_t[:, X_t_cols] + np.transpose(Phi_Theta)

    # for i in range(N):
    #
    #     rate = Phi_t[X_t_rows[i], :] * Theta_t[:, X_t_cols[i]] * X_t_values[i]
    #     Xt_to_t1_t[:, X_t_cols[i]] += rate
    #     WSZS_t[X_t_rows[i], :] += rate
...
def Sample_Phi(WSZS_t,Eta_t):   # (array, scalar)
    ...
    Phi_t = np.random.gamma(Phi_t_shape)
#    for kt in range(Kt):
